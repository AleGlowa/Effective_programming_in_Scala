...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap(
      links => WikiResult.traverse(links.toSeq)(client.nameOfArticle).map(_.toSet)
    )
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if q.isEmpty || q.head(0) > maxDepth then
        WikiResult.successful(None)
      else
        val (currNode, newQ) = q.dequeue
        val (nodeDistance, nodeId) = currNode
        val neighbors = client.linksFrom(nodeId).orElse(Set.empty)
        val isFound = neighbors.map(_.contains(target))
        isFound.flatMap(
          found =>
            if found then
              ...
              neighbors.flatMap(
                ns =>
                  iter(visited union ns, newQ.enqueueAll(ns.map((nodeDistance + 1) -> _)))
              )
        )
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    for {
      articleIds <- WikiResult.traverse(titles)(client.searchId)
      idToTitle = articleIds zip titles
      pairs =
        for {
          first <- idToTitle
          second <- idToTitle
      ...
      distances <- WikiResult.traverse(pairs) {
        case (first, second) =>
          breadthFirstSearch(first(0), second(0), maxDepth)
            .map[(String, String, Option[Int])]((first(1), second(1), _))
      }
    } yield distances
end Wikigraph
