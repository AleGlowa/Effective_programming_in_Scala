...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val highestValue: Int = x max y
      val heap1: List[Node] = insert(highestValue, insert(highestValue, empty))

      val lowestValue: Int = x min y
      val heap2: List[Node] = insert(lowestValue, insert(lowestValue, empty))
      ...
      val deleteTwoMinAndFindMin: Boolean =
        findMin(deleteMin(deleteMin(meldedHeap))) == highestValue
  ...
  val meldingHeaps: (String, Prop) =
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) =>
      def check(meldedHeap: List[Node], heap1: List[Node], heap2: List[Node]): Boolean =
        meldedHeap match {
          case h if isEmpty(heap1) && isEmpty(heap2) && isEmpty(h) =>
            true
          case h if !isEmpty(heap1) && findMin(h) == findMin(heap1) =>
            check(deleteMin(h), deleteMin(heap1), heap2)
          case h if !isEmpty(heap2) && findMin(h) == findMin(heap2) =>
            check(deleteMin(h), heap1, deleteMin(heap2))
          case _ =>
            false
        }
